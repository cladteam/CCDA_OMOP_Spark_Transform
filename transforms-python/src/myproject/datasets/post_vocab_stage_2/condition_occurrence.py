...
def compute(source_df, codemap):
    short = source_df.limit(100).collect()
    #split_source_value = F.split(source_df.condition_source_value, '\\|') # splits on a regex, escape the 'or'
    split_source_value = F.split(short.condition_source_value, '\\|') # splits on a regex, escape the 'or'
    ...
    df = short.withColumn('condition_concept_source_system', split_source_value.getItem(0)) \
                  .withColumn('condition_concept_source_code', split_source_value.getItem(1))
