...
def snoop_df(df):
    #for itm, val in df.columns:
    for column in df.items():
        column_name = column[0]
        for row in column[1]:
            if row != row:
                msg = f"SNOOP: {column_name} {type(row)} {row}\n"
                raise Exception(msg)


def convert_and_write_pandas_attempt(ctx, name, dataframe_dict, spark_ds, co_debug):
        if name in dataframe_dict:
            schema = domain_dataset_schema[name]
            my_df = dataframe_dict[name]
            if my_df is None or schema is None:
                  raise Exception(f"NULLNess going on {name}  {schema} {my_df}")

            # DEBUG 1 
            #snoop_df(my_df)

            # DEBUG 2 - no schema, different ds, only for Condition_occurrence
            # (this is basically how we got to specifying the schema...)
            #  [CANNOT_MERGE_TYPE] Can not merge type `DateType` and `DoubleType`. 
            #spark_dff_debug = ctx.spark_session.createDataFrame(my_df) 
            #co_debug.write_dataframe(spark_dff_debug)

            # DEBUG 3
            if False:
                # trying to show what's going on, one row at a time, but creating a heisenbug
                # the arg is a Series, not a DataFrame, so make it iterable with the list brackets. 
                for i in range(0, len(my_df)):
                    short_df = my_df.iloc[i].copy()
                    try:
                        #https://stackoverflow.com/questions/74710953/pyspark-createdataframe-typeerror-structtype-can-not-accept-object-id-in-type
                        #spark_dff = ctx.spark_session.createDataFrame(short_df, schema)
                        spark_dff = ctx.spark_session.createDataFrame(data=[short_df], schema=schema)
                        #spark_ds.write_dataframe(spark_dff) # in here, this creates some error that is irrelevant to figuring out if the above line works
                        # but it runs all the way throught when below barfs on a nan that must be from a NaT
                    except Exception as e:
                        df_contents = "DF: "
                        for column in short_df.index:
                            df_contents += f"{name}:{column} {type(short_df[column])} {short_df[column]}, "
                        raise Exception(f"inner: {df_contents}")
                        #raise Exception(f"inner: i:{i} {name} len:{len(short_df)} df:{short_df} exception:{e} {df_contents}")
            # REGULAR            
            else:
                # TODO learn this in detail. None, N/A are complicated and different in Pandas and Spark
                # https://www.linkedin.com/pulse/nan-nat-none-whats-difference-make-data-useful/
                # https://stackoverflow.com/questions/45181768/pandas-dataframe-to-spark-dataframe-handling-nan-conversions-to-actual-null
                # my_df.where(pd.notnull(my_df), other=None, inplace=True) WtAF? # creates a table of 0 and nulls, because that's what you're asking it to.
                # my_df.where(my_df.isna(), other=None, inplace=True) # creates a table of 0 and nulls

                spark_dff = ctx.spark_session.createDataFrame(my_df, schema) # `DateType()` can not accept object `nan` in type `float`.
                spark_ds.write_dataframe(spark_dff  )
        ...
        with fs.open(status.path, 'rb') as f:
            ...
            for match in doc_regex.finditer(contents):            
                ...
                for key in new_data_dict:
                    if key in omop_dataframe_dict and omop_dataframe_dict[key] is not None:
                        if new_data_dict[key] is  not None:
                            omop_dataframe_dict[key] = pd.concat([ omop_dataframe_dict[key], new_data_dict[key] ])
                    else:
                        omop_dataframe_dict[key]= new_data_dict[key]
                    ##if new_data_dict[key] is not None:
                        ##logger.info(f"{status.path} {key} {len(omop_datafa_dict)} {omop_dataframe_dict[key].shape} {new_data_dict[key].shape}")
                    ##else:
                        ##logger.info(f"{status.path} {key} {len(omop_dataframe_dict)} None / no data")
...
#    convert_and_write(ctx, 'Care_Site',   omop_dataframe_dict, care_site)
    convert_and_write(ctx, 'Condition',   omop_dataframe_dict, condition_occurrence, co_debug)
#    convert_and_write(ctx, 'Drug',        omop_dataframe_dict, drug_exposure)
#    convert_and_write(ctx, 'Location',    omop_dataframe_dict, location)
#    convert_and_write(ctx, 'Measurement', omop_dataframe_dict, measurement)
#    convert_and_write(ctx, 'Observation', omop_dataframe_dict, observation)
#    convert_and_write(ctx, 'Person',      omop_dataframe_dict, person)
#    convert_and_write(ctx, 'Procedure',   omop_dataframe_dict, procedure_occurrence)
#    convert_and_write(ctx, 'Provider',    omop_dataframe_dict, provider)
#    convert_and_write(ctx, 'Visit',       omop_dataframe_dict, visit_occurrence)
#    convert_and_write(ctx, 'Care_Site',   omop_dataframe_dict, care_site)
