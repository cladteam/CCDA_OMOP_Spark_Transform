...
@transform(
    omop_eav_dict = Output("/All of Us-cdb223/HIN - HIE/CCDA/IdentifiedData/OMOP_spark/omop_eav_dict_batches_more_simple"),
    previous_files = Output("/All of Us-cdb223/HIN - HIE/CCDA/IdentifiedData/OMOP_spark/omop_eav_dict_batches_record_more_simple"),
...
#        return
    ...
    def process_file(file_path, total_eav_list): # different from the flatMap way that uses a file status and yields
        """
    ...
    for file_row in input_files_list:
        process_file(file_row.path, eav_list)

    # Each file is made distinct down in data_driven_parse.py, but you still can
    # get duplicates for things like location, care_site, and provider across files. 
    # This only checks that within a batch. Faster with small batches, but won't catch
    # as much duplication.
    eav_list = make_distinct(eav_list) # gonna hurt, but will hurt worse without

    omop_eav_df = ctx.spark_session.createDataFrame(eav_list, omop_dict_schema)
